document.addEventListener('DOMContentLoaded', function() {
    ...
    function startPcmStream(sampleRate) {
        ...
        isBuffering = true;  // Add this flag
        bufferSize = 3;  // Number of chunks to buffer before starting playback
    ...
    function processPcmChunk(base64data) {
        ...
        try {
            ...
            // If we're buffering, wait until we have enough chunks
            if (isBuffering) {
                if (pcmQueue.length >= bufferSize) {
                    isBuffering = false;
                    processPcmQueue();
                }
                return;
    ...
    async function processPcmQueue() {
        ...
        try {
            ...
            if (pcmQueue.length > 0) {
                ...
                // Delay the end status slightly to ensure all audio is played
                setTimeout(() => {
                    isAssistantSpeaking = false;
                    interruptButton.disabled = true;
                }, 250);
