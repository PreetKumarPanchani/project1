...
class VoiceAssistant:
    ...
    async def speak(self, text: str):
        """Generate speech using OpenAI TTS and send to browser with better interruption and mute handling"""
        if not text:
            return
        ...
        try:
            ...
            if self.is_interrupted:
                logger.info("Speech generation completed but interrupted before sending")
                return
            ...
            await self.connection_manager.send_personal_message(
                {"type": "audio", "data": f"data:audio/mp3;base64,{audio_base64}"},
                self.websocket
            )
