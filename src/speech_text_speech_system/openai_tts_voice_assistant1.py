...
class VoiceAssistant:
    def __init__(self):
        ...
        self.listener_lock = threading.Lock()  # Added lock for listener thread
    ...
    def _speak_thread(self, text: str):
        """Thread for speaking text using OpenAI TTS with better interruption handling"""
        audio_stream = None
        pa = None
        
        try:
            with self.speech_lock:
                # Set state at beginning of critical section
                self.is_speaking = True
                self.is_interrupted = False

                # Create a new PyAudio instance for this thread
                # to avoid conflicts with the listening thread
                pa = pyaudio.PyAudio()
                ...
                try:
                    ...
                    with self.openai_client.audio.speech.with_streaming_response.create(
                        ...
                        for chunk in response.iter_bytes(chunk_size=2048):
                            # Check interruption flag at each chunk
                            ...
                            if chunk and len(chunk) > 0:
                                ...
                                # Small sleep to allow interruption to be detected
                                time.sleep(0.01)
            ...
            # Always mark speech as done, even if there was an error
            self.is_speaking = False
    ...
    def _process_audio(self, audio):
        ...
        try:
            ...
            # Check if speech is happening and immediately interrupt
            if self.is_speaking:
                logger.info("Detected speech during speaking - interrupting")
                self.is_interrupted = True
                
                # Give a small delay for speech to stop
                time.sleep(0.2)
                
                # Wait for speaking flag to be cleared by speech thread
                wait_count = 0
            ...
            # Now process the command after any interruption is handled
            ...
            while self.is_listening:
                try:
                    with self.mic as source:
                        ...
                        # Try to get input with listener lock to prevent conflicts
                        with self.listener_lock:
                            audio = self.recognizer.listen(source, timeout=2, phrase_time_limit=5)
                        
                        # Process audio - will handle interruption if needed
    ...
    def stop(self):
        ...
        self.is_listening = False
        self.is_speaking = False
        self.is_interrupted = True  # Set interrupted to ensure speech stops
